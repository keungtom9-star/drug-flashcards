...
- In Ward, open Labs and search `sodium`, `INR`, `troponin`, `HbA1c`, `lactate` and `anion gap`. Check every category chip, clear button, card expansion and the official-guidance carousel. Confirm the local-range warning remains visible, values fit without horizontal overflow, and search results update immediately.
