...
- Search for a drug missing locally but present in the latest Google Sheet; it should appear without calling AI. Then search for a drug absent from both sources: AI should return it, add it to Google Sheet once, and make it immediately searchable on the device.
- For an AI-found drug, confirm Side Effects contains useful adverse effects and never shows `Not specified`, `Not listed`, `Unknown` or `N/A`. Tap 廣東話解釋 and confirm the final answer uses short, readable Traditional Chinese Cantonese sections; English is limited to necessary drug names and medical terms.
