...
- Tap the small app icon or `AI Drug Tutor` title from Search, Quiz, Ward and Clinical. It should return to the clean main Search page without opening the keyboard. With the keyboard closed, confirm the compact four-tab bar stays directly below the header, the content reaches the lower safe area, and there is no duplicate or floating navigation.
...
- Select `Qwen3.8 27B (Free · Secure)` and confirm no API-key field appears in the browser. AI search, Cantonese explanation and quiz generation should call only `/.netlify/functions/openrouter-qwen`; browser requests must not contain an `Authorization` header. With `OPENROUTER_API_KEY` removed from Netlify, the function should return a clear configuration error without exposing any secret.
